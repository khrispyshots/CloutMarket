...
import { FEE_SELL_PCT, INITIAL_OWNED_SHARES } from '../engine/config';
import { getSellReturn, getSpotPrice } from '../engine/bondingCurve';
...
export const SellToken: React.FC<{ creatorId: string; onBack: () => void; onComplete: () => void }> = ({ creatorId, onBack, onComplete }) => {
  ...
  const heldShares = state.shareHoldings[creator.id] ?? 0;
  const priceUsd = getSpotPrice(currentSupply);
  const shares = parseFloat(shareAmount);
  const sharesValid = Number.isFinite(shares) && shares > 0 && shares <= heldShares;
  const grossReturn = sharesValid ? getSellReturn(currentSupply, shares) : 0;
  const sellFee = grossReturn * FEE_SELL_PCT;
  const netReturn = Math.max(0, grossReturn - sellFee);
  const usdEstimate = sharesValid ? netReturn.toFixed(2) : '0.00';
    ...
    <div className="h-full min-h-0 flex flex-col bg-clout-bg px-4 sm:px-5 w-full">
      ...
      <main className="flex-1 min-h-0 overflow-y-auto space-y-5 py-4 pb-6">
        <BrutalistCard variant="white" className="flex items-center gap-3 p-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 text-[10px] font-black uppercase text-slate-500 tracking-wider">
              ...
              Curve spot ${priceUsd.toFixed(2)} / share
            </div>
          </div>
        </BrutalistCard>
        ...
        <section className="text-center space-y-1">
          ...
          <p className="text-xs font-bold text-slate-500">Holdings: {heldShares.toFixed(4)} shares</p>
        </section>
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            />
            <button type="button" onClick={() => setShareAmount((heldShares / 2).toFixed(4))} className="absolute right-3 top-1/2 -translate-y-1/2 bg-slate-100 px-3 py-1.5 border border-slate-200 rounded-full text-[10px] font-black uppercase hard-shadow-sm press-interaction">
              HALF
            </button>
          </div>
        </div>
        ...
        <BrutalistCard variant="surface" className="p-4 bg-white space-y-2">
          <div className="flex justify-between text-[11px] font-bold uppercase text-slate-500">
            <span>Curve return</span>
            <span className="font-black text-border-dark">${grossReturn.toFixed(2)}</span>
          </div>
          <div className="flex justify-between text-[11px] font-bold uppercase text-slate-500">
      ...
      <div className="shrink-0 p-4 safe-bottom bg-clout-bg/95 backdrop-blur border-t border-slate-200">
        <StickerButton
          ...
          onClick={
            sharesValid
              ? () => {
                dispatch({ type: 'SellToken', creatorId: creator.id, tokenAmount: shares, usdEstimate: netReturn });
                onComplete();
              }
              : undefined
          }
        >
          {sharesValid ? `Sell ${shares} shares` : 'Enter valid amount'}
        </StickerButton>
      </div>
    </div>
  );
